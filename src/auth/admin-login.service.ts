...
export class AdminLoginService {
  ...
  private invalid(): never {
    throw new UnauthorizedException(
      'Login confirmation expired or is invalid. Sign in again.',
    );
  }
  ...
  private async assertAttemptsAvailable(userId: number) {
    ...
    if (Number(await this.redis.get(key)) >= MAX_ATTEMPTS) {
      this.errors.tooManyRequests(
        'Too many confirmation attempts. Please sign in again later.',
  ...
  private async pending(challengeId: string): Promise<PendingLogin> {
    ...
    if ((await this.redis.get(this.activeKey(pending.userId))) !== challengeId)
      return this.invalid();
    return pending;
  }
  ...
  async request(user: User) {
    ...
    return {
      ...
      message:
        'A sign-in code has been sent to your email. It expires in 5 minutes.',
      expires_in: CODE_TTL,
  ...
  async confirm(challengeId: string, code: string) {
    ...
    // Reserve attempts atomically before checking the code, including parallel requests.
    // Resending or re-entering credentials does not reset this account-wide limit.
    ...
    if (!this.hash.compareToken(`${challengeId}:${code}`, pending.codeHash)) {
      throw new UnauthorizedException({
        message: 'Incorrect login confirmation code.',
    ...
    // Only one caller can consume the challenge and create a session.
